    ...
    default:
      return (
        <li className="py-3 border-b">
          ...
          <p className="text-sm font-medium">by <Link to={`/profile-info?userId=${props.userId}`} className="underline hover:opacity-80 transition-opacity">{`${props.userId.substr(0,6)}...${props.userId.substr(-4,4)}`}</Link> Block: {props.date}</p>
        </li>
      );
  }
}
